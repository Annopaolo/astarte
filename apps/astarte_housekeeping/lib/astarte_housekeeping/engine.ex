...
defmodule Astarte.Housekeeping.Engine do
  ...
  defp update_device_registration_limit(realm_name, new_limit) do
    _ = Logger.info("Updating device registration limit", tag: "update_device_registration_limit")

    case new_limit do
      nil -> :ok
      :remove_limit -> delete_device_registration_limit(realm_name)
      n when is_integer(n) -> set_device_registration_limit(realm_name, n)
    end
  end
  ...
  defp merge_realm_status(old_realm, %{device_registration_limit: nil} = update_attrs) do
    attrs = Map.drop(update_attrs, [:device_registration_limit])
