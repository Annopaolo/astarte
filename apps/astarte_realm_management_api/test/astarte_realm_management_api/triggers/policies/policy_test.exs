...
defmodule Astarte.RealmManagement.API.Triggers.Policies.PolicyTest do
  ...
  test "install, list, source and delete succeed with valid attrs" do
    ...
    assert {:ok, [@policy_name]} = Policies.list_trigger_policies(@realm)
    ...
    assert {:error, :already_installed_trigger_policy} =
  ...
  test "delete fails when policy is not installed" do
    assert {:error, :trigger_policy_not_installed} =
             Policies.delete_trigger_policy(@realm, "pippo")
  end
