...
defmodule Astarte.DataUpdaterPlant.DataUpdaterTest do
  ...
  test "device deletion is acked and related DataUpdater process stops" do
    ...
    INSERT INTO deletion_in_progress (device_id)
    VALUES (:device_id)
    """

    deletion_in_progress_query =
      DatabaseQuery.new()
      |> DatabaseQuery.statement(deletion_in_progress_statement)
      |> DatabaseQuery.put(:device_id, device_id)

    DatabaseQuery.call!(db_client, deletion_in_progress_query)
    ...
    |> expect(:rpc_call, fn serialized_call, @vmq_plugin_destination ->
      ...
      client_id = "#{realm}/#{encoded_device_id}"
    ...
    dup_start_ack_query =
      DatabaseQuery.new()
      |> DatabaseQuery.statement(dup_start_ack_statement)
      |> DatabaseQuery.put(:device_id, device_id)

    assert [dup_start_ack: true] =
             DatabaseQuery.call!(db_client, dup_start_ack_query) |> DatabaseResult.head()
    ...
    device_row =
      DatabaseQuery.call!(db_client, msgs_query)
      |> DatabaseResult.head()
    ...
    dup_end_ack_query =
      DatabaseQuery.new()
      |> DatabaseQuery.statement(dup_end_ack_statement)
      |> DatabaseQuery.put(:device_id, device_id)

    assert [dup_end_ack: true] =
             DatabaseQuery.call!(db_client, dup_end_ack_query) |> DatabaseResult.head()
