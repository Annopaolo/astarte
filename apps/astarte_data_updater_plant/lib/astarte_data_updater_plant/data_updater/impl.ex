...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Impl do
  ...
  def handle_heartbeat(%State{drop_messages: true} = state, _, message_id, _) do
    MessageTracker.ack_delivery(state.message_tracker, message_id)
    state
  end
  ...
  def handle_internal(%State{drop_messages: true} = state, "/f", _, message_id, _) do
    {:ok, db_client} = Database.connect(realm: state.realm)
    :ok = Queries.ack_end_device_deletion(db_client, state.device_id)
  ...
  def handle_data(%State{drop_messages: true} = state, _, _, _, message_id, _) do
    MessageTracker.ack_delivery(state.message_tracker, message_id)
    state
  end
  ...
  def handle_introspection(%State{drop_messages: true} = state, _, message_id, _) do
    MessageTracker.ack_delivery(state.message_tracker, message_id)
    state
  end
  ...
  def handle_control(%State{drop_messages: true} = state, _, _, message_id, _) do
    MessageTracker.ack_delivery(state.message_tracker, message_id)
    state
  end
  ...
  def handle_install_volatile_trigger(
        %State{drop_messages: true} = state,
        _,
        message_id,
        _
      ) do
  ...
  def handle_delete_volatile_trigger(%State{drop_messages: true} = state, _, message_id, _) do
    MessageTracker.ack_delivery(state.message_tracker, message_id)
    state
  end
  ...
  defp maybe_start_device_deletion(db_client, state, timestamp) do
    if should_start_device_deletion?(db_client, state.device_id) do
      ...
      Queries.ack_start_device_deletion(db_client, state.device_id)

      %State{new_state | drop_messages: true}
    else
      state
    end
  end

  defp should_start_device_deletion?(db_client, device_id) do
    case Queries.check_device_deletion_in_progress(db_client, device_id) do
      {:ok, true} ->
        true
