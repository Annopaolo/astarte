...
defmodule Astarte.Housekeeping.API.Realms.RealmUpdate do
  use Ecto.Schema
  ...
  def changeset(realm, params \\ %{}) do
    ...
    |> validate_number(:device_registration_limit, greater_than_or_equal_to: 0)
    ...
    |> normalize_if_present(:replication_class, &replication_class_to_atom/1)
    |> normalize_if_present(:datacenter_replication_factors, &Enum.to_list/1)
  end
  ...
  defp normalize_if_present(changeset, field, normalization_function) do
    value = get_field(changeset, field)

    if value, do: put_change(changeset, field, normalization_function.(value)), else: changeset
  end
